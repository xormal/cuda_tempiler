...
class Resources(Protocol):
    ...
    # ДВА РЕГИСТРОВЫХ ЧИСЛА -- ЭТО ДВА КРАЯ ПОЛОСЫ ОДНОЙ ОЦЕНКИ, А НЕ ОДНО ЧИСЛО ДВАЖДЫ.
    #   regs ...... край, который вариант СОХРАНЯЕТ.  Им считают резидентность и границу
    #               времени: там оптимистичный край -- корректная сторона (граница обязана быть
    #               НИЖНЕЙ), и им же нельзя выбрасывать.
    #   max_live .. край, который НЕ ОБЕЩАЕТ «влезает».  Им отвечают на вопрос «разольётся ли»,
    #               потому что ошибка в сторону «влезает» ТИХО теряет победителя, а ошибка в
    #               сторону «не влезет» стоит одной лишней сборки.
    # Плагин, у которого оценка точная, кладёт в оба одно и то же число.
    def verdict(
        self,
        regs: int,
        max_live: int,
...
class Launch:
    ...
    smem_bytes: int
    entry: str
    # ОБЪЯВЛЕННАЯ СБОРЩИКУ минимальная резидентность.  LAW=L-PRUNE-REGS-PESSIMISTIC
    #
    # Это НЕ занятость и НЕ пожелание: это число, которое скелет СООБЩАЕТ сборщику, и по
    # которому сборщик назначает бюджет регистров.  Оно обязано ходить через границу, потому
    # что иначе конвейеру приходится ВЫВОДИТЬ бюджет из собственной оценки регистров -- то есть
    # проверять оценку бюджетом, посчитанным по ней же.  Круг замерен: на трёх телах из
    # двенадцати выведенный бюджет оказался НИЖЕ того, что сборщик реально взял (136 при
    # «бюджете» 128), и два варианта, собравшиеся без единого разлива, были выброшены.
    #
    # Умолчание 1 означает «резидентность сборщику не объявлена»: тогда бюджет -- потолок ISA.
    min_ctas_per_sm: int = 1
