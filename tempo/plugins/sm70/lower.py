...
class Sm70Skeletons:
    ...
    def resources_of(self, op: OpSpec, h: Hyperform):
        """(regs, max_live, smem) БЕЗ СБОРКИ.  ДВА ЧИСЛА -- ДВА КРАЯ ОДНОЙ ПОЛОСЫ.

        LAW=L-PRUNE-REGS-PESSIMISTIC.  Полоса замерена против ptxas на десяти собранных телах
        паспорта: оценка/ptxas = 0.747..1.254.  Точки в этой полосе нет и быть не может --
        предмет здесь СТРУКТУРА СКЕЛЕТА, а не машинный код, и разброс идёт от распределителя.
        Значит у модели остаётся только НАПРАВЛЕНИЕ ошибки, и края разведены ПО ЦЕНЕ ОШИБКИ:

          * `regs` -- ОПТИМИСТИЧНЫЙ край (`regs_for_verdict`).  Идёт в резидентность и в
            границу времени.  Там занижение безопасно: граница обязана быть НИЖНЕЙ, а
            выбрасывать по этому краю -- значит выбрасывать по тому, что вариант сохраняет.
          * `max_live` -- ПЕССИМИСТИЧНЫЙ край (`regs_ceiling`) минус замеренный расход сверх
            живости.  Идёт в вопрос «разольётся ли».  До задачи 144 сюда шёл ТОТ ЖЕ
            оптимистичный край, и отсекатель называл влезающими ОБА тела паспорта, которые
            ptxas разлил (кадр 8 Б), а заодно выбрасывал два, собравшиеся без разлива.

        РАСХОД СВЕРХ ЖИВОСТИ -- ставка машины (REG.OVERHEAD), а не константа эмиттера: он
        замерен лестницей на пробнике и обязан двигаться вместе с таблицей ставок.
        """
        g = space.to_gemm(h)
        over = int(_M.rate("REG.OVERHEAD").value)
        return int(g.regs_for_verdict()), int(g.regs_ceiling() - over), int(g.smem)
    ...
    def launch_of(self, op: OpSpec, h: Hyperform) -> Launch:
        ...
        return Launch(
            ...
            # ЧИСЛО, КОТОРОЕ СБОРЩИК УСЛЫШИТ.  Скелет объявляет его в теле ядра, и по нему
            # сборщик назначает бюджет регистров; сюда оно вынесено, чтобы конвейеру не
            # приходилось ВЫВОДИТЬ бюджет из проверяемой им же оценки.
            min_ctas_per_sm=g.MINB,
        )
