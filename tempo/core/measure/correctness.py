...
ORDER = (
    ...
    "4. сломать симметрию независимой сверкой по ЧУЖОМУ пути, НАЗВАВ отличающийся узел",
    "5. и только теперь -- секундомер",
)

# ШАГ 4 -- НЕ ПОЖЕЛАНИЕ, А УСЛОВИЕ ПРОХОДА (LAW=L-RELL2-NECESSARY-NOT-SUFFICIENT).
# Замерено на дефекте 145: встроенная сверка показывала relL2 = 2e-4 всё время, пока сетка несла
# мусор, -- потому что эталон считался на ТЕХ ЖЕ собранных данных.  Поэтому `ok` без названного
# чужого пути НЕ ДАЁТСЯ, и секундомер остаётся запрещён: отказ дешевле тихо неверного вердикта.
