...
def main():
    ...
    P("=" * 162)
    P(
        f"{'роль':9s} {'K':>6s} {'N':>6s} {'M':>5s} | {'cuBLAS мс':>10s} {'ТФЛОП/с':>8s} {'МГц':>5s} "
    ...
    for r in J["rows_torch"]:
        ...
        P(
            f"{r['role']:9s} {r['K']:6d} {r['N']:6d} {r['M']:5d} | "
    ...
    P(
        "          ОСТОРОЖНО: медиана NVML НЕ ГОДИТСЯ как знаменатель. Она даёт 1530 даже там, где"
    )
    P(
        "          установившаяся частота 1230-1400 (минимум по той же форме это и показывает):"
    )
    P(
        "          провалы короткие, и NVML их недоопрашивает. Надёжный знаменатель берётся из"
    )
    P(
        "          установившегося режима -- bench/data_power.cu, 1 с непрерывной нагрузки:"
    )
    P("          1230-1400 МГц при 291 Вт => пик 100.7-114.7, а не 125.3 ТФЛОП/с.")
    P(
        "%пика   -- от ПАСПОРТНЫХ 125.3 ТФЛОП/с @1530. Это НИЖНЯЯ оценка доли: по установившейся"
    )
    P("          частоте те же ядра дают 77-80 %, а не 63-68 %.")
    P(
        "%лучш   -- от лучшего, что cuBLAS взял сам на этих же формах (достижимый потолок)"
    )
    ...
    big = [
        r for r in J.get("rows_cxx", []) if r["M"] >= 128 and r["volta_ratio_med"] > 0
    ]
    small = [
        r for r in J.get("rows_cxx", []) if r["M"] < 128 and r["volta_ratio_med"] > 0
    ]
    P(
        "--- ВЕРДИКТ 1: заявка «наш плотный fp16-GEMM = 0.67-0.82x cuBLAS» -----------------------"
    )
    if big:
        v = sorted(r["volta_ratio_med"] for r in big)
        P(
            f"  ПРЕФИЛЛ (M>=128, {len(big)} точек): {v[0]:.3f}..{v[-1]:.3f}, "
            f"медиана {statistics.median(v):.3f}, геосреднее {geo(v):.3f}"
        )
    if small:
        v = sorted(r["volta_ratio_med"] for r in small)
        P(
            f"  ДЕКОД   (M<=32,  {len(small)} точек): {v[0]:.3f}..{v[-1]:.3f}, "
            f"медиана {statistics.median(v):.3f}, геосреднее {geo(v):.3f}"
        )
    npd = [
        (r["nopred_ms"] / r["volta_ms"], r)
        for r in J.get("rows_cxx", [])
        if r.get("nopred_ms", -1) > 0 and r.get("volta_ms", -1) > 0
    ]
    if npd:
        P(
            f"  цена предикации по M (тот же размер плитки без неё, {len(npd)} точек): "
    ...
    P(
        f"    >1.00  -- владеем стыком И выигрываем в марже: {len(win)} точек"
        + (
            ": "
            + ", ".join(
                f"{next((p for p, K, N in PROJ_NAMES if K == r['K'] and N == r['N']), '?')}"
                f"/M{r['M']}={r['volta_ratio_med']:.3f}"
                for r in win
            )
            if win
            else ""
    ...
    P(
        f"    0.95-1.00 -- владеем стыком ценой единиц процентов: {len(par)} точек"
        + (
            ": "
            + ", ".join(
                f"{next((p for p, K, N in PROJ_NAMES if K == r['K'] and N == r['N']), '?')}"
    ...
    P(
        "--- ВЕРДИКТ 2: НАИВНЫЙ ВХОД (нижняя отметка темполятора) ---------------------------------"
    )
    nn = [r for r in J.get("rows_cxx", []) if r.get("naive_tflops", -1) > 0]
    if nn:
        v = sorted(r["naive_ratio_med"] for r in nn)
        P(
            f"  наивный/cuBLAS: {v[0]:.4f}..{v[-1]:.4f}, геосреднее {geo(v):.4f} "
            f"-> cuBLAS быстрее входа в {1 / geo(v):.0f} раз"
        )
        vv = sorted(
            r["volta_ms"] / r["naive_ms"] for r in nn if r.get("volta_ms", -1) > 0
        )
        if vv:
            P(
                f"  рукописный мейнлуп/наивный: обгон входа в {1 / geo(vv):.0f} раз "
                f"({1 / vv[-1]:.0f}..{1 / vv[0]:.0f})"
            )
        P(
            f"  наивный на пике: {max(r['naive_tflops'] for r in nn):.3f} ТФЛОП/с = "
    ...
    P(
        "--- РЕЖИМ M<=64 (ДЕКОД): связывает ЧТЕНИЕ ВЕСОВ, а не тензорный конвейер -----------------"
    )
    P("  достижимая полоса V100-SXM2 замерена = 841 ГБ/с (docs/VOLTA_SM70.md)")
    P(
        f"  {'роль':9s} {'M':>4s} | {'cuBLAS ГБ/с':>12s} {'%полосы':>8s} | {'мейнлуп ГБ/с':>13s} "
    ...
    for r in J["rows_torch"]:
        ...
        c = cxx.get((r["K"], r["N"], r["M"]), {})
        v_gb = (
            by / (c["volta_ms"] * 1e-3) / 1e9
            if c.get("volta_ms", -1) > 0
            else float("nan")
        )
        P(
            f"  {r['role']:9s} {r['M']:4d} | {cb_gb:12.1f} {100 * cb_gb / 841:7.1f}% | "
            f"{v_gb:13.1f} {100 * v_gb / 841:7.1f}% | {100 * r['cublas_tflops'] / peak:12.1f}%"
        )
    P(
        "  ЧИТАТЬ ТАК: там, где %полосы высок, а %счёта низок, соперник НЕ В ФОРМЕ по счёту, и"
    )
    ...
    if os.path.exists(roof):
        ...
        dd = [
            x.get("ratio_dp4a_over_fp16")
            for x in R["rows"]
            if x.get("ratio_dp4a_over_fp16")
        ]
        mult = geo(rr)
        P(
            f"  ЗАМЕРЕННЫЙ множитель формата int8 (уложенного в мантиссу fp16) к fp16: "
            f"x{mult:.3f} (геосреднее по {len(rr)} боевым точкам)"
        )
        if dd:
            P(
                f"  DP4A (единственный целочисленный тракт sm_70) к fp16: x{geo(dd):.3f} "
    ...
    if big and mult:
        ...
        for r in J.get("rows_cxx", []):
            ...
            role = next(
                (p for p, K, N in PROJ_NAMES if K == r["K"] and N == r["N"]), "?"
            )
            P(
                f"  {role:9s} {r['M']:5d} {rel:7.3f} | {1.85 * rel:8.3f} | {mult * rel:9.3f} | "
