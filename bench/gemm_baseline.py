...
class ClockSampler(threading.Thread):
    ...
    def stop(self):
        self.stop_ev.set(); self.join(timeout=3)
...
def smi_procs(dev):
    ...
    out = subprocess.check_output(
        ["nvidia-smi", "-i", str(dev), "--query-compute-apps=pid,used_memory",
         "--format=csv,noheader"], text=True).strip()
    mine = str(os.getpid())
    lines = [l for l in out.splitlines() if l.strip() and l.split(",")[0].strip() != mine]
    return len(lines), "; ".join(lines)


def gpu_state(dev):
    sm, pw, ut = subprocess.check_output(
        ["nvidia-smi", "-i", str(dev), "--query-gpu=clocks.sm,power.draw,utilization.gpu",
         "--format=csv,noheader,nounits"], text=True).strip().split(", ")
    n, procs = smi_procs(dev)
    return {"sm_mhz": float(sm), "power_w": float(pw), "util": float(ut),
            "n_foreign_procs": n, "foreign_procs": procs}
...
def bench(fn, target_ms=400.0, min_iter=3, max_iter=2000):
    ...
    for _ in range(max(1, it // 3)):         # прогрев ДО стабилизации мощности, вне секундомера
        fn()
...
def make_gptq_int8(K, N, G, dev, seed):
    ...
    qz = torch.randint(-2 ** 31, 2 ** 31 - 1, (K // G, N // 4), device=dev, dtype=torch.int32)
...
def build_cxx(out_bin):
    ...
    cmd = [os.path.join(cuda_home, "bin/nvcc"), "-O3", "-std=c++17", "-arch=sm_70",
           "-Wno-deprecated-gpu-targets", "-lcublas", "-I", GEMM_INC,
           "-o", out_bin, os.path.join(HERE, "gemm_baseline.cu")]
...
def run_cxx(binpath, dev, rounds, ms_list, quiet=False):
    env = dict(os.environ)
    env["LD_LIBRARY_PATH"] = (os.environ.get("CUDA_HOME", "/home/alex/miniconda3/envs/cuda128")
                              + "/lib:" + env.get("LD_LIBRARY_PATH", ""))
    cmd = [binpath, "--dev", str(dev), "--rounds", str(rounds),
           "--m", ",".join(str(m) for m in ms_list), "--naive-budget", "6000"]
...
def main():
    ...
    print(f"# ДО:  {st_before['sm_mhz']:.0f} МГц, {st_before['power_w']:.0f} Вт, "
    ...
    if not args.skip_cxx:
        ...
        for r in cxx_rows:
            print(f"  cxx K{r['K']:6d} N{r['N']:6d} M{r['M']:5d}  cuBLAS {r['cublas_tflops']:7.2f} "
    ...
    for role, K, N in PROJ:
        ...
        for M in ms_list:
            ...
            Wl = torch.randn(N, K, device=dev, dtype=torch.float16) * 0.05    # раскладка F.linear
            flop = 2.0 * M * N * K
            ...
            rec = {"role": role, "K": K, "N": N, "M": M, "flop": flop,
            ...
            if w8 is not None:
                ...
                for tag, fn in (("w8a16_gemv", lambda: ext.w8a16_gemv(x, qw, qz, sc, 128)),
            ...
            if rec["clock"]:
                ...
                rec["pct_of_peak"] = 100 * rec["cublas_tflops"] / rec["peak_at_own_clock"]
            ...
            rows.append(rec)
            print(f"  {role:9s} K{K:6d} N{N:6d} M{M:5d}  cuBLAS {cb:9.4f} мс "
    ...
    print(f"# ПОСЛЕ: {st_after['sm_mhz']:.0f} МГц, {st_after['power_w']:.0f} Вт, "
          f"чужих процессов {st_after['n_foreign_procs']} {st_after['foreign_procs']}")
    if all_clk:
        print(f"# частота ПОД НАГРУЗКОЙ: мин {all_clk['min']:.0f} / медиана {all_clk['med']:.0f} / "
              f"макс {all_clk['max']:.0f} МГц ({all_clk['n']} отсчётов)")
    fmed = all_clk["med"] if all_clk else 1530.0
    out = {
        "gpu": name, "dev": args.dev, "rounds": args.rounds,
        "seconds": round(time.time() - t_start, 1),
        "state_before": st_before, "state_after": st_after,
        "clock_under_load": all_clk, "clock_torch_arms": clk_torch, "clock_cxx_arms": clk_cxx,
        "peak_tflops_1530": PEAK_PER_MHZ * 1530,
        "peak_tflops_at_measured_clock": PEAK_PER_MHZ * fmed,
        "valid_no_neighbour": st_before["n_foreign_procs"] == 0 and st_after["n_foreign_procs"] == 0,
        "rows_torch": rows, "rows_cxx": cxx_rows, "cxx_skips": cxx_skips,
    }
