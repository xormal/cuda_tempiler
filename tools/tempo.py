...
ВАЖНО, ЧТО MIO -- РЕСУРС SM, А НЕ ПЛАНИРОВЩИКА: его ёмкость объявлена НА ВЕСЬ SM, а ёмкость
прочих каналов -- НА ПЛАНИРОВЩИК, поэтому нагрузка MIO домножается на ВСЕ варпы, а прочих -- на
варпы НА ПЛАНИРОВЩИК (W/4).

ЧТО ЗДЕСЬ СТОЯЛО И ОПРОВЕРГНУТО (закон L-OCCUPANCY-MOVES-BINDING, data/laws/method.json).
Отсюда делался вывод "связывающий ресурс МЕНЯЕТСЯ с занятостью". Он неверен, и опровергает его
арифметика ЭТОГО ЖЕ файла: обе нагрузки ЛИНЕЙНЫ по числу варпов (x W и x W/4), значит их
ОТНОШЕНИЕ от занятости не зависит. Замер подтвердил: прогон одного тела при 8/16/32 варпах дал
одни и те же доли каналов трижды -- DRAM 100 %, MIO 36 %, ВЫДАЧА 26 %, TENSOR 25 %.
Занятость входит в вердикт по-настоящему только через регистровый бюджет Q(W) и через дыру, где
модель молчит (задержка при малом числе варпов -- канала для неё НЕТ).
Само правило остаётся в силе: "связывающий ресурс надо открывать, а не назначать".
...
# --------------------------------------------------------------------------------------------
# ТЕ ЖЕ ОПЕРАНДЫ, НО ПО ШИРИНЕ (задача 141, долг 1).  LAW=L-REG-WIDTH-NOT-NAMES
#
# ПОЧЕМУ ОТДЕЛЬНАЯ ПАРА ФУНКЦИЙ, А НЕ ПРАВКА ПРЕЖНИХ.  dst_of/srcs_of отвечают на вопрос «какие
# ИМЕНА участвуют» -- он нужен графу зависимостей, и там расширение по ширине было бы ЛИШНИМ
# (дуга между R24 и R26 не появляется оттого, что они соседи).  Живость же спрашивает другое:
# «сколько ЯЧЕЕК занято», и вот там имя -- неверная единица.  Две разные задачи -- две функции.
def dst_regs_of(ins):
    """Приёмник, расширенный по ширине операнда: `LDS.U.128 R24` -> R24..R27."""
    d = dst_of(ins)
    if d is None:
        return []
    return IS.expand_reg(d, IS.reg_span(ins)[0])


def src_regs_of(ins):
    """Источники, расширенные по ширине: у адреса своя ширина, у данных своя."""
    dw, sw, aw = IS.reg_span(ins)
    out = set()
    for r in srcs_of(ins):
        out.update(IS.expand_reg(r, aw if r in ins.addrregs else sw))
    return out
