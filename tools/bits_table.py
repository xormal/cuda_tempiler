...
# Здесь берётся 841 и ТОЛЬКО ОНО, потому что доли BW_FRAC ниже сняты ПРОТИВ НЕГО же
# (EV_mridge.md): подставить сюда 878 значило бы поделить замеренные доли на другую базу и
# получить сдвиг 4.4 % во ВСЕЙ таблице разрядности. Замена одного числа другим здесь -- НЕ
# уточнение, а рассогласование с собственным замером; правильное лечение -- подпись режима.
ROOF_GBS = 841.0  # РЕЖИМ: чтение С РЕДУКЦИЕЙ, sum() по 256 МБ fp32. База BW_FRAC.
ROOF_GBS_READ_PURE = 878.0  # РЕЖИМ: чистое последовательное ЧТЕНИЕ. НЕ база BW_FRAC.
ROOF_GBS_COPY = 819.0  # РЕЖИМ: ЧТЕНИЕ И ЗАПИСЬ, copy_ 256 МБ fp32.
...
ROOF_TFLOPS_ = 93.6
HMMA_MAC_PER_INSTR = (
    ROOF_TFLOPS_ * 1e12 / 2 / SLOTS_PER_S
)  # ~95.6 MAC на выданную инструкцию
...
MODELS = {
    ...
    "llama": dict(
        path="/mnt/d2/dnld/Llama-3.2-1B-Instruct",
        vlm=False,
        shapes=LLAMA_SHAPES,
        layers=LLAMA_LAYERS,
    ),
...
class Quant:
    ...
    def __init__(
        ...
        self.scale_fmt, self.lut, self.clip, self.outlier = (
            scale_fmt,
            lut,
            clip,
            outlier,
        ...
        self.plane_base, self.plane_mode, self.plane_read = (
            plane_base,
            plane_mode,
            plane_read,
        )
    ...
    def find_params(self, w):
        ...
        if self.scale_fmt == "pow2":
            ...
            if self.sym or self.lut is not None:
                ...
                z = torch.round(-w.amin(1, keepdim=True).clamp_max(0) / s).clamp(
                    0, self.qmax
                )
    ...
    def _clip_search(self, w, s, z):
        ...
        for f in [1.0, 0.95, 0.9, 0.85, 0.8, 0.75, 0.7]:
            ...
            q = torch.clamp(
                torch.round(w / st) + z,
                self._lo(),
                self.qmax if not self.sym else self.qmax,
            )
    ...
    def quantize_matrix(self, W):
        ...
        if self.lut is not None:
            self._cb = (
                _lloyd_codebook(
                    W / W.abs().amax(1, keepdim=True).clamp_min(1e-12), self.bits
                )
                if self.lut == "lloyd"
                else _nf_codebook(self.bits, W.device)
            )
            ...
            if self.lut is not None:
                ...
                if self.plane_base:
                    ...
                    if self.plane_read == "A":
                        ...
                        q = (
                            torch.floor(q / (1 << drop))
                            if self.plane_mode == "or"
                            else torch.round(q / (1 << drop))
                        ) * (1 << drop)
...
def sites_of(L):
    ...
    return [
        ("qkv", [getattr(sa, n, None) for n in ("q_proj", "k_proj", "v_proj")]),
...
class Harness:
    def __init__(self, args):
        ...
        try:  # transformers >= 5 принимает dtype, старые -- torch_dtype
            ...
            self.model = M.from_pretrained(cfg["path"], torch_dtype=torch.float16).to(
                "cuda"
            )
        self.model.eval()
            ...
            for site, mods in sites_of(L):
                for k, m in enumerate(mods):
                    if m is not None:
                        ...
                        self.master[(li, site, k)] = (
                            m.weight.data.detach().to("cpu").clone()
                        )
        print(
            f"# мастер-копия: {sum(v.numel() for v in self.master.values()) * 2 / 2**30:.1f} ГиБ в ОЗУ",
            flush=True,
        )
...
def mk_quant(s):
    return Quant(
        ...
        group=s.get("group", 0),
        sym=s.get("sym", True),
        scale_fmt=s.get("scale_fmt", "fp16"),
        lut=s.get("lut"),
        clip=s.get("clip", False),
        plane_base=s.get("plane_base", 0),
        plane_mode=s.get("plane_mode", "signed"),
        plane_read=s.get("plane_read", "AB"),
    )
...
def apply_scheme(h, spec, cal_chunks=None, verbose=True):
    ...
    if comp == "gptq":
        ...
        for band in bands:
                ...
                for site, mods in sites_of(h.layers[li]):
                    ...
                    for m in mods:
                        ...
                        if q.lut is not None:
                            q._cb = (
                                _lloyd_codebook(
                                    W / W.abs().amax(1, keepdim=True).clamp_min(1e-12),
                                    q.bits,
                                )
                                if q.lut == "lloyd"
                                else _nf_codebook(q.bits, W.device)
                            )
            ...
            if verbose:
                print(
                    f"    полоса {band[0]}..{band[-1]} готова, {time.time() - t0:.0f} с",
                    flush=True,
                )
    else:
        for li in todo:
...
def scheme_sets(names):
    ...
    if name in ("main", "all"):
        ...
        add(
            "план A один; ИЛИ-раскладка (усечение)",
            bits=10,
            group=128,
            sym=False,
            plane_base=8,
        ...
        add(
            "БОЕВОЙ рецепт как в проде (L0 и qkv в fp16)",
            bits=8,
            group=128,
            sym=False,
            comp="gptq",
            sites=["o", "gate_up", "down"],
            layers=(1, 10**9),
        )
    if name in ("mods", "all"):
    ...
    if name in ("bias2", "all"):
        add(
            "МОД bias-точный: int8-g128-asym-rtn",
            bits=8,
            group=128,
            sym=False,
            bias_corr=True,
        ...
        add(
            "МОД bias-точный: int4-g128-asym-rtn",
            bits=4,
            group=128,
            sym=False,
            bias_corr=True,
        ...
        add(
            "МОД bias-точный: int3-g128-asym-rtn",
            bits=3,
            group=128,
            sym=False,
            bias_corr=True,
        ...
        add(
            "МОД bias-точный: int6-g128-asym-rtn",
            bits=6,
            group=128,
            sym=False,
            bias_corr=True,
        ...
        add(
            "МОД bias-точный: план A ИЛИ (усечение)",
            bits=10,
            group=128,
            sym=False,
            plane_base=8,
    ...
    if name in ("layers", "all"):
        ...
        add(
            "кроме-Q1 (Q1 в fp16)", bits=3, group=128, sym=False, layer_frac=(0.25, 1.0)
        )
    if name in ("budget", "all"):
        # РАСПРЕДЕЛЁННЫЙ БЮДЖЕТ (то, что даёт прогрессивное чтение)
        add(
            "budget-A: down8 rest4",
            bits=4,
            group=128,
            sym=False,
    ...
    if name in ("budget2", "all"):
        ...
        add(
            "budget-F: o8 qkv7 down6 gate_up5",
            bits=6,
            group=128,
            sym=False,
        ...
        add(
            "budget-G: o7 qkv6 down5 gate_up4",
            bits=5,
            group=128,
            sym=False,
...
def cmd_run(args):
    ...
    print(
        f"# калибровка: {len(cal_chunks)} x {args.callen} = "
        f"{len(cal_chunks) * args.callen} токенов",
        flush=True,
    )
    done = {}
    ...
    schemes = [
        s
        for s in scheme_sets(args.set)
        if not args.only or s["tag"] in args.only.split(",")
    ]
    ...
    for spec in schemes:
        ...
        print(
            f"    flip_tf = {100 * bad / tot:.2f} %  [{100 * lo:.2f}; {100 * hi:.2f}]  "
            f"N={tot}  ({row['secs']} с)",
            flush=True,
        )
        rows.append(row)
...
def cmd_planes(args):
    ...
    out["int10_planes_roundtrip_all1024"] = bool(torch.equal(back, u)) and bool(
        torch.equal(val, (1024 + u).float())
    )
    ...
    pb2 = u2 - 4 * pa2
    out["int10_signed_split_ok_0_1021"] = bool(torch.equal(4 * pa2 + pb2, u2)) and bool(
        pb2.min() >= -2 and pb2.max() <= 1
    )
    ...
    for M in (32, 64):
        ...
        res[M] = dict(
            fp16_ms=t_fp16 * 1e3,
            int8_1pass_ms=t_one * 1e3,
            planes_4p4_2pass_ms=t_two * 1e3,
            base4_only_ms=t_base * 1e3,
    ...
    t2 = bench(
        lambda: a.sum(dtype=torch.int32) + b.sum(dtype=torch.int32), it=20, warm=5
    )
    out["two_streams"] = dict(
        one_ms=t1 * 1e3,
        two_ms=t2 * 1e3,
        gbs_one=n / t1 / 1e9,
        gbs_two=n / t2 / 1e9,
...
def cmd_selftest(args):
    ...
    chk(
        "бит/вес int4-g128-asym = 4.15625",
        abs(b - (4 + 16 / 128 + 4 / 128)) < 1e-9,
        f"{b}",
    )
    b2, _ = bits_per_weight(dict(bits=8, group=0, sym=True), 3840, 15360)
    chk(
        "бит/вес int8-row-sym = 8+16/3840",
        abs(b2 - (8 + 16 / 3840)) < 1e-9,
        f"{b2:.5f}",
    )
    b3, _ = bits_per_weight(
        dict(bits=4, group=64, sym=False, scale_fmt="pow2"), 3840, 15360
    )
    chk(
        "бит/вес int4-g64-pow2 < int4-g64-fp16",
        b3 < bits_per_weight(dict(bits=4, group=64, sym=False), 3840, 15360)[0],
        f"{b3:.4f}",
    )
    ...
    for spec in (
        dict(bits=8, group=0, sym=True),
        dict(bits=4, group=128, sym=False),
        ...
        chk(
            f"RTN {spec} ошибка <= шаг",
            float((Q - W).abs().max()) <= float(step) * tol,
    ...
    chk(
        "два прохода по плоскостям == один по int8 (аккумулятор fp32)",
        r < 1e-6,
        f"rel норма={r:.2e}",
    )
    ...
    print(
        f"       [справочно] с округлением каждого прохода до fp16: "
    ...
    chk(
        "GPTQ <= RTN по взвешенной ошибке",
        eg < er,
        f"rtn={er:.1f} gptq={eg:.1f} ({eg / er:.3f}x)",
    )
    ...
    el = float(
        (Quant(bits=4, group=128, sym=True, lut="lloyd").quantize_matrix(g) - g).norm()
    )
    chk(
        "LUT(Ллойд) <= равномерного на гауссовом весе",
        el < eu,
        f"uni={eu:.1f} lut={el:.1f}",
    )
    ...
    # LAW=L-INT12-IS-ONE-ELEMENT: знаковое 12-разрядное лежит в ОДНОМ элементе половинной
    # точности, и это ИСЧЕРПЫВАЮЩИЙ перебор, а не выборка -- одно расхождение убивает схему.
    chk(
        "0x6400|u == 1024+u для всех u в 0..1023 (=> ОДИН OR даёт 10 бит)",
    ...
    chk(
        "fp16 точен по ВСЕМ целым до 2048, 2049 уже нет",
        first_bad == 2049,
    ...
    chk(
        "asym int12 (код 4095) НЕ представим точно",
        float(torch.tensor(4095.0).half()) != 4095.0
        or float(torch.tensor(4095.0).half()) == 4096.0,
        f"{float(torch.tensor(4095.0).half())}",
    )
    ...
    # 8. ТРИ РЕЖИМА ПОЛОСЫ И ЦЕНА НЕНАЗВАННОГО (LAW=L-BW-THREE-MODES, задача 118).
    # Проверяется не значение, а ДИСЦИПЛИНА: режимов три, они различны, и подстановка чужого
    # режима в базу этих же долей сдвигает ВСЮ таблицу на замеренные 4.4 %.
    trio = (ROOF_GBS_READ_PURE, ROOF_GBS, ROOF_GBS_COPY)
    chk(
        "три режима полосы объявлены и различны",
        len(set(trio)) == 3 and trio == (878.0, 841.0, 819.0),
        f"чтение/сумма/копия = {trio}",
    )
    chk(
        "цена неназванного режима = 4.4 % на ВСЕЙ таблице",
        abs((ROOF_GBS_READ_PURE / ROOF_GBS - 1) * 100 - 4.4) < 0.05,
        f"{(ROOF_GBS_READ_PURE / ROOF_GBS - 1) * 100:.2f} %",
    )
    # ДОЛИ BW_FRAC СНЯТЫ ПРОТИВ 841: доля выше единицы означала бы, что база чужая.
    chk(
        "доли полосы согласованы со СВОЕЙ базой (ни одна не выше единицы)",
        all(0.0 < v <= 1.0 for d in BW_FRAC.values() for v in d.values()),
        f"максимум {max(v for d in BW_FRAC.values() for v in d.values()):.3f}",
    )
...
def cmd_table(args):
    ...
    groups = {
        "ОСНОВНАЯ ТАБЛИЦА": [],
        "ЧУВСТВИТЕЛЬНОСТЬ (что во что кладём)": [],
    ...
    for r in rows:
        ...
        k = (
            "МОДИФИКАТОРЫ"
            if t.startswith("МОД")
            else "РАСПРЕДЕЛЁННЫЙ БЮДЖЕТ"
            if t.startswith("budget")
            else "ЧУВСТВИТЕЛЬНОСТЬ (что во что кладём)"
            if t.startswith(("only-", "кроме-"))
            else "ОСНОВНАЯ ТАБЛИЦА"
        )
        groups[k].append(r)
    ...
    print()
    print(
        "ДНО ЛЕСТНИЦЫ по формам (бит/вес, ниже которых время не падает; c = int8-цепочка):"
    )
    print("| форма | M=32 | M=64 |")
    print("|---|--:|--:|")
    for name, K, N, m in shapes:
        print(
            f"| {name} | {floor_bits(name, 32, 1.0):.2f} | {floor_bits(name, 64, 1.0):.2f} |"
        )


def _print_rows(rows, shapes, layers, nw, t16_32, t16_64):
    print(
        "| разложение | бит/вес | по слову | ГБ | мс M=32 | x | мс M=64 | x | инстр/эл |"
        " дно, бит | flip_tf | 95 % |"
    )
    print("|---|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|")
    for r in sorted(rows, key=lambda x: (x.get("flip_tf", 0))):
        ...
        bw = (
            sum(
                mix_bits(r, spec, name, K, N)[0] * m * K * N for name, K, N, m in shapes
            )
            / nw
        )
        bwp = (
            sum(
                mix_bits(r, spec, name, K, N, pack="word")[0] * m * K * N
                for name, K, N, m in shapes
            )
            / nw
        )
        ...
        print(
            f"| {r['tag']} | {bw:.3f} | {bwp:.3f} | {gb:.1f} | {t32:.4f} | {t16_32 / t32:.2f} |"
