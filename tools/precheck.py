...
def _run(cmd, **kw):
    return subprocess.run(
        cmd, capture_output=True, text=True, timeout=kw.pop("timeout", 120), **kw
    )
...
def check_imports(py, mods, pythonpath):
    ...
    for m in mods:
        p = _run(
            [py, "-c", f"import {m}; print(getattr({m},'__version__','?'))"], env=env
        )
        out.append(
            (
                m,
                p.returncode == 0,
                (p.stdout or p.stderr).strip().splitlines()[-1][:80]
...
def build_env(bdir):
    ...
    out.append(
        (
            "CUDA_HOME",
            bool(cu) and os.path.exists(nvcc or ""),
            f"{cu or '(не задан)'}"
            + ("" if not cu else f", nvcc {'есть' if os.path.exists(nvcc) else 'НЕТ'}"),
        )
    )
    if bdir:
        ...
        if os.path.exists(bdir):
            ...
            if st.st_uid != os.getuid():
                ok, why = (
                    False,
                    f"владелец uid={st.st_uid}, а мы uid={os.getuid()} (след sudo-прогона?)",
                )
...
def gpu_free(card):
    p = _run(
        [
            ...
            "-i",
            str(card),
            "--query-gpu=memory.total,memory.used,clocks.sm",
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=0,
        help="длина контекста, под которую считать KV",
    )
    ...
    if a.mods:
        for m, ok, ver in check_imports(
            a.python, [x for x in a.mods.split(",") if x], a.pythonpath
        ):
    ...
    if a.model:
        ...
        if err:
            ...
            for k, v in sorted(per.items(), key=lambda kv: -kv[1])[:6]:
                mark = (
                    "  <- БАЛЛАСТ для текстового замера"
                    if any(t in k for t in ("vision", "audio", "mm_", "multi_modal"))
                    else ""
                )
    ...
    if a.need_ctx and not a.kv_bytes_per_token:
        print(
            f"{BAD}  KV под {a.need_ctx} токенов НЕ ОЦЕНЕНА: не задан --kv-bytes-per-token"
        )
        bad.append(
            "оценка KV пропущена -- задайте --kv-bytes-per-token, посчитав ПОСЛОЙНО"
        )
    if a.need_ctx and a.kv_bytes_per_token:
        kv = a.need_ctx * a.kv_bytes_per_token
        print(
            f"{OK}  KV под {a.need_ctx} токенов: {kv / 2**30:.1f} ГиБ "
    ...
    if a.card is not None:
        ...
        if g is None:
            ...
            fr = foreign(a.card)
            print(
                f"{OK if not fr else BAD}  карта {a.card}: {used}/{tot} МиБ занято, {clk} МГц, "
            ...
            if fr:
                bad.append(
                    f"на карте {a.card} чужие процессы -- замер ВРЕМЕНИ недействителен"
                )
            if need:
                ...
                print(
                    f"{verdict}  нужно ~{need / 2**30:.1f} ГиБ + 15 % запаса, свободно "
                    f"{free_b / 2**30:.1f} ГиБ"
                )
                if free_b <= need * 1.15:
                    bad.append(
                        f"памяти не хватит: нужно ~{need * 1.15 / 2**30:.1f}, свободно "
                        f"{free_b / 2**30:.1f} ГиБ"
                    )
    ...
    if bad:
        ...
        return 1
    print(
        "ОКРУЖЕНИЕ ГОДНО. (Это не значит, что замер осмыслен -- только что он состоится.)"
    )
    return 0
