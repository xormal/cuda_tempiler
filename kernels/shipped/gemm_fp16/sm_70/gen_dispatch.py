...
def main():
    ...
    def line(cond, tag, BM, BN, BK, WM, WN, MINB):
        return (
            "  if (%s)\n    return launch_one<%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%s,%d>(p, s);  // %s"
            % (cond, BM, BN, BK, WM, WN, C["STAGES"], C["GSTAGE"], C["FPREF"], C["GROUP"],
               C["EPI"], C["SWZ"], "true" if C["PRED"] else "false", MINB, tag)
        )

    for m_max, tag, BM, BN, BK, WM, WN, MINB in m.LADDER:
        out.append(line("p.M <= %d && (p.N %% %d) == 0 && (p.K %% %d) == 0" % (m_max, BN, BK),
                        tag, BM, BN, BK, WM, WN, MINB))
    tag, BM, BN, BK, WM, WN, MINB = m.WIDE
    out.append(line("(p.N %% %d) == 0 && (p.K %% %d) == 0" % (BN, BK),
                    tag + "  (M > 256, N кратно 256)", BM, BN, BK, WM, WN, MINB))
    tag, BM, BN, BK, WM, WN, MINB = m.NARROW
    out.append(line("(p.N %% %d) == 0 && (p.K %% %d) == 0" % (BN, BK),
                    tag + "  (M > 256, узкое N)", BM, BN, BK, WM, WN, MINB))
    out.append("")
